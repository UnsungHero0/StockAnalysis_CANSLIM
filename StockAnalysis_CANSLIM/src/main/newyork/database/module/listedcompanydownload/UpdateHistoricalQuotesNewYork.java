...
public class UpdateHistoricalQuotesNewYork {

	public static void updateHistoricalQuotesNewYork(Connection con) {
		// TODO
	...
	public static void updateOneCompanyQuotes(String local_code, Connection con) {
		...
		if (JDBCUtil.hasTable(tableName, con)) {
			...
			// 6. create new quotes table
			DownLoadHistoricalQuotesNewYork.createOneHistoricalQuotesTable(local_code,
					con);
			consolePrint.println(local_code + " is new created quotes table");
		}
	...
	private static ArrayList<String> getExistDate(String code, Connection con) {
		...
		String getCodeListSql = "SELECT Date FROM "
				+ namespace.NewYorkDBNameSpace.getSchemaDb() + code
				+ namespace.NewYorkDBNameSpace.getStockhistoricalpriceDb();
		ResultSet rs = null;
		try {
